...
| `maxed` | `surface.sunken` | none (`Transparency` 1) | `content.muted` | `Max` | edge absent, fill luma 27.2 versus 51.0, the word |
...
Rec.601 luma **16.2** against cleared stone floored at **165**, a separation of ~149; the value
is 23 px carrying a 2 px `surface.sunken` text stroke; the label is 14 px carrying a hairline one.
The binding constraint at 70 px is **vertical fit**, not contrast: 14 + 23 + two chip paddings
must sit inside 70, and the same stack must sit inside 120 on tablet without the type growing,
because the ramp is fixed. That is arithmetic `viewport` and `composition` own the inputs to, and
it is `[playtest unknown]` at exactly one point: whether a 4 px versus 1 px edge is discriminable
at 70 px on a phone held at arm's length. **Test range 3 to 5 for `stroke.heavy`**, settled by
`npm run render -- --brief ui-forge/briefs/hud.brief.json --viewport all` against a
`fantasy-ornate` context, which has never been run `[research: CLAUDE.md]`.
...
```json
{
  ...
  "value": {
    ...
    "legibilityAnswer": {
      ...
      "contrast": { "plateLuma": 16.2, "clearedStoneFloor": 165, "separation": 148.8 },
...
- **VFX and Audio** inherit nothing: a purchase is `theme/tone/03` `B4`, audio and UI only, no
  world cue, and this sheet adds no cue of its own.
...
closes: `navigation` and `composition`. The token values behind every ref above, and the
forbidden-ref list: sheet `02`. Type sizes: sheet `03`. The press scale, its duration and its
easing: sheet `06`. Whether a fifth pressable exists: `input`'s owner, via `navigation/03`'s RR-3;
this sheet is written for four and does not reserve a fifth. What a purchase sounds like: Audio.
