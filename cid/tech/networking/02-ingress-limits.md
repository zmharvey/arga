...
The server accepts at most **10 `BuyUpgrade` messages per second per player with a burst of 20**, and **1 `RequestState` invoke per second per player with a burst of 3**. Anything over either ceiling is **dropped in silence** — no cue, no warning, no `warn`, no kick, no per-message log. `BuyUpgrade` returns without touching state; `RequestState`, being a `RemoteFunction`, returns `nil` immediately without building a snapshot. Counters are two token buckets per `UserId`, held in a `server-main` module-local map, live only, never persisted.
...
```manifest
{
  ...
  "value": {
    ...
    "algorithm": {
      ...
      "refillModel": "continuous, computed from os.clock() at read time; no per-tick refill job",
      "costPerMessage": 1,
    ...
    "counterStorage": {
      ...
      "keyedBy": "UserId",
      "shape": { "buyTokens": "number", "buyLastRefillClock": "number", "stateTokens": "number", "stateLastRefillClock": "number" },
    ...
    "forbidden": [
      ...
      { "id": "I2", "rule": "no warn, print or error on the over-limit path", "closedBy": "a log a modified client can fill on demand is a second denial of service", "observable": "grep the over-limit branch for warn/print/error returns zero" },
      { "id": "I3", "rule": "no kick, ban, throttle-with-notice or connection close", "closedBy": "the response ladder is tech/security integrity.response and is not this sheet's", "observable": "grep game/src/server for player:Kick returns zero" },
    ...
    "serverCostAdded": {
      "perAcceptedMessage": "one map lookup, one subtraction, one comparison, one assignment",
...
- **Whoever owns `server-main` (`architect`, `modules`)** gains one map with the same lifetime as `spawnPoses`, and one branch at the top of the `BuyUpgrade` handler and the `RequestState` invoke. It gains no `PlayerState` field.
...
- **Analytics work** gets nothing to instrument from this sheet and should not assume it does. There is no ingress event, no drop counter and no pipe.
...
2. A grep of the over-limit branches in `game/src/server/` for `warn`, `print`, `error` and `Kick` returns zero matches.
...
Whether a message the bucket accepted is valid, its argument count, its types or its byte length (`tech/security`, `integrity.channels[]`). What a violation does and what is recorded (`tech/security`, `integrity.response` and `integrity.logging`). The position rule the payout is measured against (`tech/security` sheet 01). The channel names, classes and directions (`architect/05-interfaces`). What crosses server→client and how often (sheet `01`, this domain). The client-side debounce value itself (`input`, `gameplay/mechanics/02`). Whether the client's boot pull exists at all (`architect/07-wiring`, client boot).
