...
| **exclusive rewards** | `03-META.md` *"Never content access"*; `discovery.repeat.possible: false`; `collection` closed at 24 names; `endgame.forbidden[secondCollection, secondCurrency]`; `modifiers` owns every permanent stat change and its resolution order | `sum(len(collection.sets[i].relics)) == 24` and every name reachable by clearing alone — **`collection` holds no `total` field and none may be cited**, ruled at Live Ops verification round 3 and confirmed against `bridge/schema.mjs:208-217`, whose `collection` shape is `className`, `classPlural`, `relicsPerArea`, `areasPerDepth`, `sets`; count of grants in the build conditioned on a date, a window or an event id: **0** |
