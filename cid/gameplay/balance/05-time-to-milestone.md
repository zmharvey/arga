...
proposed together as `pacing`.** The collection finishes at **17.9 minutes** for a base player
and **10.2** for a `Span` owner; the ladder empties at 26.2 / 15.0. Two corrections ride with it:
`core-loop/04`'s "three to seven complete laps" was computed on the **arrival** lap and the
realised lap is 0.868× that, so a bound session is **four to eight**; and
...
- **The realised lap is not the arrival lap, and that is the whole of the lap-count correction.**
  ...
  1,276.8 s are walked in 1,107.7 — a ratio of **0.868**. The consequence is one extra lap at
...
- **The 12% margin is replaced by two named ones, and their values are set by how much slack the
  ...
  **0.05**. They are not larger because of the post-terminal bay: one fixed size must serve a
  player from an under-buy arrival at τ 1,622.9 to a maxed ladder with all four sets and the
  product at τ 3,802.2, a **1.138× footprint window inside a 2.67× band**. Sized at its geometric
  centre — 42 chunks — the bay meets both, at 0.061 and 0.068; the eight areas clear them at
  0.167 and 0.094. **They now bind on the same row, so they are one constraint wearing two
  names**; the two were kept apart because they read different quantities and a later change to
  either end will move only one of them.
- **`UNDERBUY_LEVELS` is 1 and it holds at every row.** `depths.invariants[9]` requires the lap
  ...
  case is the post-terminal bay at 186.3 s and the worst area is ordinal 5 at 181.2 s.
- **The 90-second ceiling is carried by purchases, not by reveals, and this reverses my first
  ...
  consecutive reveals can sit **2/3 of a lap apart — 101.5 s at area 7**, past the ceiling. What
  closes it is the ladder: sheet `03` buys 5 to 7 rungs in every area, a purchase every **18.6 to
  30.5 seconds**, and 4 rungs at 42.0 s in an endless bay. `pacing` therefore carries both the
  reveal maximum and the purchase cadence, and `solvency` carries `rungsBought >= 4` as a test.
  **`rungsBought` is the difference between consecutive summed `arrivalLevels` — 6/7/7/7/7/6/5/5
  — and this sheet's previous draft published a different, wrong count.**
- **The 3-second tick ceiling is kept, with one exclusion named.** A moving player at base stats
  ...
  walk-back** from the last cleared patch to the inward opening, which at sheet `03`'s bay lengths
  is 840 studs at area 8 and 1,260 at the endless bay — about 27 s and 38 s of unpaid ground.
  That is bay geometry and belongs to `meta/06`; it is named here because this sheet publishes
  the figure that would otherwise hide it.
...
| # | arrival lap | under-buy lap | realised lap | cumulative | rungs | purchase cadence | reveal gap, mean | reveal gap, max |
|---|---|---|---|---|---|---|---|---|
| 1 | 163.6 s | 163.6 s | 141.5 s | 141.5 s | 6 | 23.6 s | 47.2 s | 94.3 s |
| 2 | 153.1 s | 180.0 s | 130.0 s | 271.5 s | 7 | 18.6 s | 43.3 s | 86.7 s |
| 3 | 154.4 s | 176.7 s | 131.9 s | 403.4 s | 7 | 18.8 s | 44.0 s | 87.9 s |
| 4 | 161.0 s | 180.0 s | 132.7 s | 536.1 s | 7 | 19.0 s | 44.2 s | 88.5 s |
| 5 | 164.0 s | 181.2 s | 147.1 s | 683.2 s | 7 | 21.0 s | 49.0 s | 98.1 s |
| 6 | 158.4 s | 173.1 s | 135.9 s | 819.1 s | 6 | 22.7 s | 45.3 s | 90.6 s |
| 7 | 162.0 s | 175.7 s | 152.3 s | 971.4 s | 5 | 30.5 s | 50.8 s | **101.5 s** |
| 8 | 160.3 s | 172.9 s | 136.3 s | 1,107.7 s | 5 | 27.3 s | 45.4 s | 90.9 s |
| bay | 173.7 s | 186.3 s | 167.9 s | — | 4 | 42.0 s | — | — |
...
| set 3, Vault | 785.1 s | 448.6 s | 75% through area 6 |
| **24 of 24, set 4, Spire** | **1,073.6 s** | **613.5 s** | 75% through area 8 |
| area 8 complete | 1,107.7 s | 632.9 s | — |
| ladder exhausted | 1,569 s | 897 s | 2.8 endless bays past area 8 |
...
```manifest
{
  ...
  "value": {
    ...
    "realisedToArrivalRatio": 0.868,
    "realisedToArrivalDerivation": "1107.7 s realised over 1276.8 s of summed arrival laps; NOT 1107.7/1320, which divides by 8 * lapTargetSeconds and is a different quantity",
    "routeSlack": 2.0, "routeSlackTestRange": [1.5, 2.6], "routeSlackStatus": "playtest unknown",
    "underbuyLevels": 1,
    "purchaserFloorMarginTarget": 0.06, "purchaserFloorMarginRealised": 0.061, "purchaserFloorMarginBindingRow": "post-terminal bay",
    "arrivalCeilingMarginTarget": 0.05, "arrivalCeilingMarginRealised": 0.068, "arrivalCeilingMarginBindingRow": "post-terminal bay",
    "marginTargetTestRange": [0.05, 0.25],
    "marginNote": "replaces the 12% figure standing after wave 3, which was the residue of withdrawing two value passes. Both now bind on the same row and are one constraint wearing two names; they are kept apart because they read different quantities (a maxed purchaser against the floor, an under-buying base player against the ceiling) and a later change moves only one. They are what sizes solvency.postTerminalBay at 42 chunks.",
    "tickGapMaxSeconds": 3.0, "tickGapRealisedSeconds": 1.17,
    "tickGapExcludes": "the end-of-lap walk-back from the last cleared patch to the inward opening: 840 studs / 27 s at area 8 and 1260 studs / 38 s at the endless bay. Bay geometry, owned by gameplay/meta/06; stated here because this key publishes the lengths.",
    "aboveTickGapMaxSeconds": 90.0,
    "aboveTickGapRealisedSeconds": 42.0,
    "aboveTickGapCarriedBy": "upgrade purchases, at 18.6 to 30.5 s inside the eight areas and 42.0 s in an endless bay",
    "revealGapMeanSeconds": 45.4,
    "revealGapMaxSeconds": 101.5,
    "revealGapNote": "at relicsPerArea 3 with one Find per contiguous third, two consecutive reveals can sit 2/3 of a lap apart. 101.5 s at area 7 EXCEEDS the 90 s ceiling, so reveal spacing does not carry the rule alone and the cost curve is load-bearing for it. solvency.tests.S5 is the guard.",
    ...
    "completeLapsCorrection": "core-loop/04 states three to seven. That figure divides the session by the ARRIVAL lap; the realised lap is 0.868x it. Same shape, one lap more at each end.",
    "rungsBoughtSource": "solvency.areaLedger[].rungsBought, which is the difference between consecutive summed arrivalLevels. Never counted independently here.",
    "laps": [
      ...
      { "ordinal": 2,     "arrivalLapSeconds": 153.1, "underbuyLapSeconds": 180.0, "realisedLapSeconds": 130.0, "cumulativeSeconds": 271.5,  "rungsBought": 7, "purchaseCadenceSeconds": 18.6, "revealGapMeanSeconds": 43.3, "revealGapMaxSeconds": 86.7 },
      ...
      { "ordinal": 4,     "arrivalLapSeconds": 161.0, "underbuyLapSeconds": 180.0, "realisedLapSeconds": 132.7, "cumulativeSeconds": 536.1,  "rungsBought": 7, "purchaseCadenceSeconds": 19.0, "revealGapMeanSeconds": 44.2, "revealGapMaxSeconds": 88.5 },
      { "ordinal": 5,     "arrivalLapSeconds": 164.0, "underbuyLapSeconds": 181.2, "realisedLapSeconds": 147.1, "cumulativeSeconds": 683.2,  "rungsBought": 7, "purchaseCadenceSeconds": 21.0, "revealGapMeanSeconds": 49.0, "revealGapMaxSeconds": 98.1 },
      { "ordinal": 6,     "arrivalLapSeconds": 158.4, "underbuyLapSeconds": 173.1, "realisedLapSeconds": 135.9, "cumulativeSeconds": 819.1,  "rungsBought": 6, "purchaseCadenceSeconds": 22.7, "revealGapMeanSeconds": 45.3, "revealGapMaxSeconds": 90.6 },
      { "ordinal": 7,     "arrivalLapSeconds": 162.0, "underbuyLapSeconds": 175.7, "realisedLapSeconds": 152.3, "cumulativeSeconds": 971.4,  "rungsBought": 5, "purchaseCadenceSeconds": 30.5, "revealGapMeanSeconds": 50.8, "revealGapMaxSeconds": 101.5 },
      { "ordinal": 8,     "arrivalLapSeconds": 160.3, "underbuyLapSeconds": 172.9, "realisedLapSeconds": 136.3, "cumulativeSeconds": 1107.7, "rungsBought": 5, "purchaseCadenceSeconds": 27.3, "revealGapMeanSeconds": 45.4, "revealGapMaxSeconds": 90.9 },
      { "ordinal": "bay", "arrivalLapSeconds": 173.7, "underbuyLapSeconds": 186.3, "realisedLapSeconds": 167.9, "cumulativeSeconds": null,   "rungsBought": 4, "purchaseCadenceSeconds": 42.0, "revealGapMeanSeconds": null, "revealGapMaxSeconds": null }
    ],
    "milestones": [
      ...
      { "id": "setComplete3",       "baseSeconds": 785.1,  "purchaserSeconds": 448.6, "unit": "s", "measuredFrom": "join",       "source": "75% through area 6" },
      { "id": "collectionComplete", "baseSeconds": 1073.6, "purchaserSeconds": 613.5, "unit": "s", "measuredFrom": "join",       "source": "75% through area 8; this is 24 of 24 and set 4 together" },
      { "id": "area8Complete",      "baseSeconds": 1107.7, "purchaserSeconds": 632.9, "unit": "s", "measuredFrom": "join",       "source": "laps[7]" },
      { "id": "ladderExhausted",    "baseSeconds": 1569.0, "purchaserSeconds": 897.0, "unit": "s", "measuredFrom": "join",       "source": "2.75 post-terminal bays past area 8, at solvency.postTerminalBay.incomePerBay" }
    ],
    ...
    "invariants": [
      ...
      "every laps[].rungsBought equals the corresponding solvency.areaLedger[].rungsBought",
...
| `core-loop/04` | depth-1 lap target | "one depth-1 lap targets 165 seconds" | keep 165 s, restate as the **arrival** lap and add the realised figure, 141.5 s | `depths.sizingRule` reads it as the arrival lap and the sheet reads it as wall clock; they differ by 13.2% |

The endless bay's 42 chunks follow from the two margin targets above and are **already carried in
...
session by the arrival lap; the realised lap is 0.868× it because throughput rises inside an
area, and eight areas take 1,107.7 s rather than 1,276.8. A 600-second session therefore holds
...
inside minute 1"); the range has none. **The cost of honouring the range instead:** `costBase`
scales all twenty rungs of an axis, so a 177 floor slows the whole trajectory by about a round
per area, income for areas 1 to 7 falls by roughly a third, the ladder is bought to under 30% at
24 of 24, and it flips `value` from the cheapest axis to the dearest, which changes the greedy
purchase order rather than shifting it. That is a regime change bought with one unsourced number.
...
genuinely load-bearing half. A purchaser still reaches `firstSpendAffordable` before
`tierContrast`; no `costBase` inside `firstPurchaseBand` prevents that, since 45 s of purchaser
clearing is 348 currency against the band's own 206.9 ceiling. It is stated in the beat list
...
- **Payoff-cadence work (`core-loop/01`)** gets its two ceilings back as data with the realised
  ...
  not by reveal spacing, because the worst reveal gap is 101.5 s. A later change to the cost
  curve can reopen it, which `solvency.tests.S5` guards.
- **Bay-geometry work (`meta/06`)** inherits two figures it did not have: the end-of-lap
  walk-back at 840 and 1,260 studs, which `tickGapRealisedSeconds` explicitly excludes, and the
  precision horizon moving to the 14th endless bay.
...
- **Endgame work (`endgame`)** gets the figure its guard needs: the above-tick rule is scoped off
  at 1,073.6 s, and **a second scoping is needed at ladder exhaustion (1,569 s)**, after which a
  post-terminal bay's only above-tick payoff is its completion, 167.9 s apart.
- **Audio and feedback work** gets 0.35 s as the coincident onset separation and a reveal heard
  24 times over 17.9 minutes.
- **Meta & Content** should note that the whole specced game is 18.5 minutes. R-3 declined
...
1. Every `laps[].realisedLapSeconds` is between 75 and 200, and every `laps[].underbuyLapSeconds`
   is at most 200 with a margin of at least 0.05 — the minimum is 186.3 s at the endless bay.
2. Every `laps[].rungsBought` equals the corresponding `solvency.areaLedger[].rungsBought`
   (6/7/7/7/7/6/5/5), every `laps[].purchaseCadenceSeconds` equals
   `realisedLapSeconds / rungsBought` and is at most 90 — the maximum is 42.0 s at the endless
   bay, against a maximum `revealGapMaxSeconds` of 101.5 s which alone exceeds it.
3. `laps[3].cumulativeSeconds` (536.1) ≤ 600 < `laps[4].cumulativeSeconds` (683.2), and
   `laps[7].cumulativeSeconds` (1,107.7) ≤ 1,200 < 1,107.7 + `laps[bay].realisedLapSeconds`
   (1,275.6) — four complete laps at the session floor, eight at the ceiling.
