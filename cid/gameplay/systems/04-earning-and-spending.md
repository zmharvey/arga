...
- **Because each completion already pays, in its own kind.** An area pays permanence — "the
  ...
  modifier (sheet `06`). A reveal pays the collection record (sheet `05`). Adding currency on
  top would pay one event twice, and would hand Balance three more numbers to invent for a
  moment that is already the loudest in the game.
...
- **The floor is 1, and its job is not rounding.** `gameplay/core-loop/01-payoff-frequency`
  ...
  per patch across a 140-patch area. `MINIMUM_PAYOUT = 1` at `Clearing.luau:38` is annotated in
  place as a config gap the emitter has no key for `[research: game/src/server/Clearing.luau]`.
  It is correct, and it now has one.
- **Rounding is `floor`, never round-half-up.** This is the divergence worth naming: over a
  140-patch area, rounding half up pays systematically more on every fractional product, which
  is a payout difference rather than a rounding artefact. The floor and the clamp commute here
  — `max(1, floor(x))` and `floor(max(1, x))` agree for every `x` this formula can produce — so
  the expression above is stated once and should appear in the code exactly once.
- **Max ladder has no sink and cannot be given one.** Every candidate is closed: rebirth
  ...
  duplicate conversion `[brief: soft]` refused twice by name, cosmetics declined in
  `03-META.md`, content access forbidden there as "**Never content access**". So the honest
  answer is that **there is no sink past max**, and the design's protection is that the ladder
  should outlast the collection. That is a requirement Balance can test, and it is in the
  manifest as one.
...
```manifest
{
  ...
  "value": {
    ...
    "authority": "server only; the client sends an upgrade id and never a cost, an amount or a balance",
    "faucetCount": 1,
    "faucets": [
      {
        "id": "patch-clear",
    ...
    "atMaxLadder": {
      ...
      "readoutHidden": false,
      "balanceFrozen": false,
      "requirementOnBalance": "the total cost of maxing all three axes must exceed the total currency yielded by clearing the areas needed to complete all four sets, so the ladder outlasts the collection",
      "unownedConsequence": "what the game is once the ladder and the collection are both finished belongs to content-structure work; this sheet states only that no currency sink appears there"
    },
    "forbidden": [
...
- **Balance & Tuning** inherits one requirement rather than three: cost the ladder so maxing all
  three axes costs more than completing the collection yields. It sets `costBase`, `costGrowth`,
  `maxLevel` and `perLevel`; it does not get a completion lump to tune, because there is none.
...
- **Offer-ladder work (currently Monetization)** may sell a multiplier and may never sell
  currency. A Robux-to-currency rate is the first step toward buying completion, which
  `03-META.md` forbids by name.
...
Both alternatives pay one moment twice, and (b) in particular converts a permanent bonus into a
one-off, which is the weaker of the two things the brief asked a completed set to be.
...
domain). What the currency is called (`02-the-currency`, same domain, already supplied). What is
worth clearing and what a Find is worth to the player (`05-the-find-ledger`, same domain). What
content exists past 24 of 24 and past max ladder (content-structure work, currently Meta &
Content). How the player physically commits a purchase on a phone (input and purchase-surface
work — the pack's known gap; I state that a purchase is a currency debit and nothing about the
verb that triggers it).
