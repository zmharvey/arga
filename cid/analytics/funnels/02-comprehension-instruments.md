...
**Every window here is join-relative or event-relative and none is measurable today**, for the
reason `01` states as a hard requirement: no join instant and no run ordinal exist in `stateShape`.
That does not change a disposition — a proxy is a proxy whether or not a clock exists — so each row
carries `readableToday` separately from its disposition.
...
| 7 | `depth` | a player who entered area 2 from a *later* run's saved state, until `runOrdinal` exists |
...
- **Onboarding work** keeps all seven `teaching[]` rows, their `taughtBy`, `byBeat`, `evidence`,
  ...
  none of them.
- **Dashboard-and-target work (KPI)** selects from six scored rows, not seven, and inherits the rule
  that `currency` and `upgradeAxes` carry no number of their own.
...
4. No entry with `independent: false` or `sameInstrumentAs` set carries a pass mark in
   `funnels.thresholds[]` (sheet `04`).

## Not decided here

What any beat teaches, when, and by which surface (`onboarding/03`, approved; `onboarding/02` holds
`firstSession`). The step order and the funnel API (`01`, this domain). Every pass mark, alarm and
action for these rows (`04`, this domain). Whether the eighth channel `sets` needs is built, and its
shape (event-catalog and protocol work). Event names and payload fields for the five read sites
(event-catalog work). What the collection surface looks like and how a group of six reads with one
slot filled (UI/UX — Screens). The join instant and run ordinal that make five of these windows
formable (state-shape work; persistence work holds the request).
