...
| custom fields per event | **3**, and only `CustomField01/02/03.Name` are honoured — *"Anything other than CustomField01.Name, CustomField02.Name, and CustomField03.Name is ignored"* | `[research: https://create.roblox.com/docs/production/analytics/custom-fields]` | a fourth field is not an error, it is silent data loss. This is the cap that binds the whole catalog: it is why `slot_claimed` is a second event name rather than a fourth field on `session_start`, and why `area_cleared`'s `lap` field is a five-value composite encoding three facts in one slot rather than three fields |
| combined unique values | **8,000** across all custom fields, after which *"values will be grouped as 'Other'"* | `[research: https://create.roblox.com/docs/production/analytics/event-types]` | 954 used (`9 × 2 × 53`), leaving about 7,000 of headroom — which is why the five-value `lap` composite costs nothing worth counting. Forbids any free-text or unbounded field — sheet `02` `N24` |
...
- **Session and lap-clock work** should read the combined-value row above as the reason the
  five-value `lap` composite is affordable: about 7,000 of headroom against a field that costs 3
  more values than the two-value form and preserves two exclusions it would otherwise lose.
...
4. `telemetry.budget` carries `eventNameCap` 100, `customFieldCap` 3, `combinedValueCap` 8000 and
   `retentionDays` 90; `combinedValuesUsed` 954 is under the cap; and the worst-minute sum
   (`runtime.maxPlayers × 4` join requests plus steady-state in-play traffic) is at most
   `perServerRequestsPerMinute`.

## Not decided here

Which events exist, their ids, sites, payloads and required/optional fields — sheet `01`, this
domain, which holds the key these values ride in and defines the `lap` field in `sharedPredicate`.
What no event may ever carry — sheet `02`. The clock, the gap derivation, what resets it and the
per-session pass predicate — sheet `04`. `batchPatchCount`, its flush list and every
`LogEconomyEvent` argument value — economy-flow work; this sheet supplies the share and the
denominator and sets no window. `runtime.maxPlayers` — per-server-capacity work. Funnel names, step
ordinals and pass marks — funnel-definition work. What each `lap` value means for the published
distribution — session and lap-clock work. Whether the transport is `AnalyticsService` at all, and
how the pipe batches, stores or fails — logging-pipeline work. Whether `ingressLimits` `I2` should
expose a counter — networking work. Every figure in `pacing`, `solvency` and `collection` this
inequality is evaluated over — Balance and Meta, and wave 4 has not released.
