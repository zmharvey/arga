...
```manifest
{
  ...
  "value": {
    ...
    "composition": {
      ...
      "toolAreaPct": 0,
      "toolAreaPctBecause": "the HEAD bridges the two regions and the GRIP excludes nothing. objectArt's head is [178,160,133], luma 162.30 (styleGuide roles wood.worked adopted verbatim): 162.30 - 123.11 = 39.19 over tiers[0] and 195.84 - 162.30 = 33.54 under stone.built, both inside L2's 40-wide band, so banding it with the green leaves 33.54 to the stone, banding it with the stone leaves 39.19 to the green, and leaving it alone is a third region against L3's cap of 2. Every assignment fails L4. The grip [118,88,66] luma 94.46 is 28.65 below tiers[0], inside one band with it, and adds no region: it is not a ground. styleGuide 01 formerly ratified the head as [190,158,118] luma 163.01, the near-duplicate objectArt declined by name; that paragraph was corrected this round to [178,160,133] / 162.30 / 67.84 and the two sheets now agree. At the superseded figure the separations were 39.90 and 32.83 and the verdict was unchanged either way, which is why this row did not depend on the outcome.",
      "toolHeadLumaDiscrepancy": {
        "objectArtValue": 162.30,
        "objectArtRgb": [178, 160, 133],
        "styleGuideProseValue": 162.30, "styleGuideProseValueWas": 163.01, "styleGuideProseValueCorrectedThisRound": true,
...
- **Tool and palette work**: the conflict I recorded here is **closed**, in the direction I read
  it. `art/style/01` ratified the tool head as `[190,158,118]` luma 163.01 while `art/objects/03`,
  which owns the part, holds `[178,160,133]` luma 162.30 — and `art/style/01`'s own
  `roles["wood.worked"]` already held 162.30, so its prose was ratifying a value its own role
  table contradicted. That paragraph now carries the owning value and the two sheets agree.
  I read the owning value throughout, and the icon's verdict was the same at both, so nothing
  here ever waited on it. **Recording a conflict you are not the owner of, and reading the
  owning value meanwhile, is what made this cheap to close.**
...
| ~~**The tool head's two luma values.**~~ **RESOLVED as (a) this round.** `art/style/01` said 163.01, `art/objects/03` says 162.30 for the same `Part`. | (a) `objectArt` is authoritative and `styleGuide`'s prose is corrected; (b) the reverse; (c) leave both | **(a), applied.** `styleGuide.roles["wood.worked"].appliesTo` already said the tool is `objectArt`'s to set, so the prose was ratifying a value its own role table contradicted. `art/style/01` now carries 162.30. Neither figure changed this icon. |
