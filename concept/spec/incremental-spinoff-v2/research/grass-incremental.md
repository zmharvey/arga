...
## Corrections found on the second pass

**The pacing number is still unverified, and a search snippet nearly supplied a false one.**
A search summary reported "first rebirth around 15-20 minutes". Fetching the page showed it
actually says *"around Level 31 with 25-100 Prestige Points"* and the fetch flagged it as
generic advice rather than measured data. The minutes figure was the search engine's
invention. **First rebirth is level-gated, not time-gated; the duration remains unknown.**
[research: https://findingdulcinea.com/how-to-play-grass-incremental/]

**That same guide is probably about the wrong game.** It describes reset layers named
Prestige (L31) -> Crystallize (L101) -> Grasshop (L201) and five currencies (Grass, XP,
Prestige Points, Platinum, Flowers). A three-layer nested-reset structure matches *Grass
Cutting Incremental* (LethalDolphin, 5+ reset layers), not the reference. The guide has
likely conflated the two similarly-named games -- the exact collision this sheet warns about
above. **None of its detail has been folded into the spec.**

Three source types were tried for the pacing number: a wiki-style guide (405), a beginner
guide site, and a progression guide site. Still unresolved.
