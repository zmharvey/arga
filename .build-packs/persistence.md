...
### `layout` — `game/src/shared/Layout.luau`

Compose one area from authored chunks and place its patches, tiers and Finds, deterministically from runtime.layoutSeed and the area ordinal alone.
...
**The module:** `game/src/server/Persistence.luau`

Luau, `--!strict`, returning a table. Match the conventions in `GameConfig.luau`.

**The report:** `game/src/server/Persistence.report.md`
