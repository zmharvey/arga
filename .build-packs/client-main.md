...
### `hud-binding` — `game/src/client/HudBinding.luau`

Resolve the ui-forge HUD's named readouts once and write live state into them, including which readouts are withheld.

- `bind(root, gui)`

### `pressables` — `game/src/client/Pressables.luau`

Create and maintain the four game-drawn pressables, and report an activation. THE ONE MODULE IN THIS BUILD THAT AUTHORS UI STRUCTURE, and it authors exactly four buttons.

- `bind(gui, onActivate)`

### `index-screen` — `game/src/client/IndexScreen.luau`

The collection surface: which Finds are held, grouped by set, opened and closed by the index pressable.

- `bind(gui)`
- `toggle()`

### `beats` — `game/src/client/Beats.luau`

Receive the payoff channels and schedule them as beats: rank, queue, minimum onset gap, channel exclusivity and overload behaviour.

- `connect(gui)`

### `input` — `game/src/client/Input.luau`

Adjudicate the two game-bound verbs: turn a pressable activation into a BuyUpgrade message or an index toggle. Binds no key as the only path to either.
...
**The module:** `game/src/client/init.client.luau`

Luau, `--!strict`, returning a table. Match the conventions in `GameConfig.luau`.

**The report:** `game/src/client/init.client.report.md`
